...
// 赢法数组，用来存储所有的赢法
var wins=[];  //赢法数组是一个三维数组
for(var i=0;i<15;i++){  //初始化这个三维赢法数组
    wins[i]=[];
    for(var j=0;j<15;j++){
        wins[i][j]=[];
    }
}
var count=0;  //统计一共有多少种赢法
// 统计所有的横线，5个相连，则获胜
for(var i=0;i<15;i++){
    for(var j=0;j<11;j++){  //横向5个相连即获胜，所以11到15，5个相连，也到了棋盘边界。所以这里是11
        for(var k=0;k<5;k++){
